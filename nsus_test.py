...
# 제출 함수
def post_to_google_sheets(text, answer_type):
    payload = {"response": text.strip(), "type": answer_type}
    try:
...
def passage_write_step():
    ...
    if disabled:
        st.warning("Time is up. Please click [Submit Answer] to proceed.")
...
def email_write_step():
    ...
    if disabled:
        st.warning("Time is up. Please click [Submit Answer] to finish.")
